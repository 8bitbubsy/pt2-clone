...
function compile() {
    ...
    if [ $USE_HOMEBREW_SDL -eq 1 ]; then
        ...
        clang $VERBOSE $CFLAGS -I"$sdl2_include" -g0 -DNDEBUG \
            src/tinyfiledialogs/*.c src/gfx/*.c src/*.c \
        ...
        clang $VERBOSE $CFLAGS -F "$SDL2_FRAMEWORK" -g0 -DNDEBUG \
            src/tinyfiledialogs/*.c src/gfx/*.c src/*.c \
            -Wall -Winit-self -Wextra -Wunused -Wredundant-decls \
...
codesign -s - --deep --force ${APP_DIR} 2>/dev/null || {
...
echo "Output: ${APP_DIR}Contents/MacOS/pt_pal_editor-macos"
echo ""
...
rm src/gfx/*.o src/*.o src/tinyfiledialogs/*.o &> /dev/null || true
echo "Done!"
